...
Clipboard commands are **always default-deny** and require an explicit `builtin_bridge` policy. All desktop hosts support <code>text/plain</code> through the existing text clipboard path. macOS system WebView, macOS Chromium, and Windows system WebView also support <code>text/html</code>, <code>text/rtf</code>, and <code>application/rtf</code>.
